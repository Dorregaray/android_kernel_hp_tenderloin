...
static int vfe31_config_axi(int mode, uint32_t *ao)
{
	...
	msm_io_w(bus_cmd, vfe31_ctrl->vfebase +
					V31_AXI_BUS_CMD_OFF);
	...
	struct msm_isp_cmd *cmd)
{
	...
	case VFE_CMD_CAPTURE_RAW:
		...
		rc = vfe31_configure_pingpong_buffers(VFE_MSG_V32_CAPTURE,
	...
	case VFE_CMD_ZSL:
		rc = vfe31_configure_pingpong_buffers(VFE_MSG_V32_START,
			VFE_MSG_OUTPUT_PRIMARY);
		if (rc < 0)
			goto proc_general_done;
		rc = vfe31_configure_pingpong_buffers(VFE_MSG_V32_START,
...
static inline void vfe31_read_irq_status(struct vfe31_irq_status *out)
{
	...
	msm_io_w(out->vfeIrqStatus0,
		vfe31_ctrl->vfebase + VFE_IRQ_CLEAR_0);
	msm_io_w(out->vfeIrqStatus1,
		vfe31_ctrl->vfebase + VFE_IRQ_CLEAR_1);
